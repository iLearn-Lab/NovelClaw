...
class CompositiveExecutor:
    ...
    def __init__(self, config: Config):
        ...
        # 初始化外部静态知识库（存储大量小说/创意文本数据）
        self.static_kb = StaticKnowledgeBase(config)
        ...
        # 初始化所有Agent（传入memory_system和static_kb）
        self.agents = {
            ...
            "evaluator": EvaluatorAgent(config, self.llm_client, self.retriever, self.memory_system, self.static_kb),
...
You are writing segment {idx+1}/{seg_count} of a long English story. Continue the narrative with clear structure (setup/rising/climax/resolution across segments), grounded characters, coherent worldbuilding. Avoid headings/markdown/bullets. Aim ~{seg_words} words. Maintain continuity with prior segments via memory; do not restart the story."""
    ...
    def generate_long_text(
        ...
        while iteration < max_iterations:
            ...
            # 静态库是否启用（只首轮使用时，后续关闭）
            use_static_kb = True
            if getattr(self.config, "static_kb_first_only", False) and iteration > 1:
                use_static_kb = False
            ...
            if target_length and planned_total:
                chapter_target = max(
                    getattr(self.config, "min_chapter_chars", 1000),
            ...
            # 强制单章长度下限：max(配置最小字数, 本章目标的95%)
            chapter_min_required = max(
                getattr(self.config, "min_chapter_chars", 0),
                int(chapter_target * 0.95) if chapter_target else 0
            )
            # 不再自动重写；仅提示不足，要求后续章节注意
            if story_len < chapter_min_required:
                print(f"[Chapter] 警告：本章长度 {story_len} 低于要求 {chapter_min_required}，请在后续章节补足总字数。")
            ...
            # 记录进度（字数、章节）
            try:
                self._write_progress_log(self.chapter_counter, story_len, planned_total, topic)
            except Exception:
                pass
            
            # 评估生成内容（按配置间隔，首章/末章必评）
            eval_interval = getattr(self.config, "eval_interval", 0)
            do_eval = (eval_interval <= 1) or (iteration == 1) or (iteration == max_iterations) or (eval_interval and iteration % eval_interval == 0)
            if do_eval:
                print("[Evaluator] 正在评估生成内容...")
                evaluation = self.agents["evaluator"].evaluate_multiple(round_results_for_eval)
                evaluation_result = evaluation.get("evaluation", {})
            else:
                evaluation_result = {
                    "coherence_score": 0.8,
                    "emotional_score": 0.8,
            ...
            # 一致性分数影响奖励
            consistency_score = consistency_result.get("overall_confidence", 0.5)
            reward["consistency_score"] = consistency_score
            reward["total_reward"] = (
                reward["total_reward"] * 0.8 + consistency_score * 0.2
            )
            
            if do_eval:
                print(f"[Reward] 奖励分数: {reward['total_reward']:.3f}")
                print(f"  - 新颖性: {reward['novelty_score']:.3f}")
                print(f"  - 连贯性: {reward['coherence_score']:.3f}")
                print(f"  - 整体: {reward['overall_score']:.3f}")
            ...
            if reward["total_reward"] > best_reward:
                ...
                best_result = {
                    ...
                    "round_results": round_results_for_eval,  # 包含修改后的结果
            ...
            # 判断是否继续
            if not self.reward_system.should_continue(
                reward,
                iteration,
                target_length=target_length,
                current_length=len(story_text)
            ):
                print(f"[Executor] 质量达标，停止迭代")
                break
            
            # 调整执行计划
            print("[Organizer] 根据反馈调整执行计划...")
            execution_plan = self.organizer.adjust_plan(
                execution_plan,
                {"evaluation": evaluation_result, "reward": reward},
                iteration
            )
    ...
    def _build_agent_prompt(
        ...
        if agent_name == "plot":
            ...
            min_words_hint = f"本章必须不少于 {getattr(self.config, 'min_chapter_chars', 3000)} 字，请一次性写足，避免低于下限。"
            return base_prompt + (
...
"""
    ...
    def _write_progress_log(self, chapter_no: int, words: int, planned_total: int, topic: str):
        """
        ...
        with open(path, "a", encoding="utf-8") as f:
            f.write(f"chapter={chapter_no}, words={words}, planned_total={planned_total}, topic={topic}\n")
